...
class neuralNetwork:
    ...
    def rejectThink(self): 
        ...
        tempLink , tempNeuralID = self.steplinkList [ index ] , self.stepList [ index ]
 
        
        i , o , w , nid = tempLink[0] , tempLink[1] , tempLink[2] , tempLink[3]
    ...
    def endThink(self,winIND):     # winIND = Y/N
        ...
        lensteplinkList = len(self.steplinkList)  
      
        for x1 in range(  lensteplinkList ):
        ...
        if winIND=="N": 
            inputI = self.steplinkList[lensteplinkList-1][0]
            outputI = self.steplinkList[lensteplinkList-1][1]
            weightI = self.steplinkList[lensteplinkList-1][2]
            outputNID = self.steplinkList[lensteplinkList-1][3]
            neuralID = self.stepList[lensteplinkList-1] 
            self.brain[neuralID].linkList.loc[ (self.brain[neuralID].linkList["I"] == inputI) & (self.brain[neuralID].linkList["O"] == outputI) ,["W"] ] = round(  weightI *  self.rejectRate, self.awardRound )
...
class neural:
    ...
    def procces(self,inputI):
        ...
        # search the input
        tempD = self.linkList[self.linkList["I"] == inputI ]  #faster?
        #tempD = self.linkList.query(" I == @inputI")         #lower?
        #print(tempD)
        ...
        #return [ inputI, random.randint(1, 9) , 500 , None  ]
        ...
        #
        weight , temp , i = tempD["W"].tolist() , tempD["O"].tolist() , inputI
        ...
        w , nid= tempD[tempD["O"] == o ]["W"].tolist()[0] , tempD[tempD["O"] == o ]["NID"].tolist()[0]
